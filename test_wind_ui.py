...
check("OFFなら入力を検証しない（不正な値でも通る）",
      app.build_wind_args(False, "x", "abc", "abc", "abc", "abc", "abc", "abc") == {"enabled": False})
...
check("仙台・高圧: 東北・託送2.15・手数料3.0（推定）", "東北" in n1 and "2.15" in n1 and "3.0" in n1 and "推定" in n1, n1)
n2 = app.wind_area_note("14163 (SAPPORO)", False, "特別高圧")
check("札幌・特高: 北海道・託送1.02・手数料1.5", "北海道" in n2 and "1.02" in n2 and "1.5" in n2, n2)
...
check("入力の末尾は『太陽光を使う』＋風力8欄（WIND_INPUT_KEYS と同じ並び）",
...
check("末尾の欄のラベルが 太陽光・風力・容量の指定方法・契約容量・カバー率・設備利用率・PPA単価・託送・手数料 の順",
      "太陽光発電を使う" in tl[0] and "風力発電を併用する" in tl[1] and "容量の指定方法" in tl[2] and "契約容量" in tl[3]
      and "需要カバー率" in tl[4] and "設備利用率" in tl[5] and "PPA単価" in tl[6] and "託送" in tl[7] and "小売手数料" in tl[8],
      str(tl))
check("既定: 太陽光ON・風力OFF・容量の指定・設備利用率29.1・PPA単価11.96",
      tail[0].value is True and tail[1].value is False and tail[2].value == app.WIND_SIZING_CAPACITY
      and tail[5].value == 29.1 and tail[6].value == 11.96, str([c.value for c in tail]))
check("任意入力（容量・カバー率・託送・手数料）は空欄で始まる（Numberにしない）",
      all(t.value in (None, "") for t in (tail[3], tail[4], tail[7], tail[8])))
...
def click_with(station=sendai, **over):
    ...
    m = dict(pv=-9, enabled=-8, sizing=-7, capacity=-6, coverage=-5, cf=-4, ppa=-3, wheeling=-2, fee=-1)
...
_a = list(defaults); _a[station_idx] = sendai; _a[_fac] = "なし"; _a[-8] = True; _a[-6] = "500"
...
m = re.search(r"風力: (\S+)エリア ([\d,.]+)kW, 設備利用率 ([\d.]+)%, PPA ([\d.]+)円/kWh, 託送\(従量\) ([\d.]+)円/kWh, "
              r"小売手数料 ([\d.]+)円/kWh, 太陽光=(\S+)", dbg)
check("各欄の値が正しい欄に届く（容量500・利用率25・PPA10・託送3・手数料1）",
      bool(m) and m.groups() == ("東北", "500.0", "25.0", "10.00", "3.00", "1.00", "使用"), str(m.groups() if m else dbg[-200:]))
...
w_kwh = float(re.search(r"年間発電量（風力）: ([\d,.]+) kWh/年", o[4]).group(1).replace(",", ""))
...
check("月別: 太陽光・風力の積み上げ＋需要・自家消費の4系列", names == ["発電量（太陽光）", "発電量（風力）", "需要量", "自家消費（風力の配達分を含む）"], str(names))
...
check("日別: 合計の『発電量』に、うち太陽光・うち風力の線が加わる", "発電量" in dn and "うち太陽光" in dn and "うち風力（発電量）" in dn, str(dn))
idx = [i for i, md_ in enumerate(st["month_day"]) if md_ == (7, 1)][0]
wt = [t for t in day.data if t.name == "うち風力（発電量）"][0]
...
check("日付を変えての再描画（保存した結果から）でも内訳が出る", any(t.name == "うち風力（発電量）" for t in redraw.data))
