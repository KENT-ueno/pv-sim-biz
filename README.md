...
- **風力発電（オフサイトPPA）** — 太陽光に加えて風力を第2の発電源として使えます（太陽光のみ／風力のみ／併用）。**北海道・東北のみ**（需要地も同じエリアに限り、越境託送はモデル化していません）。風力＝契約容量×設備利用率×形状で、形状は各エリアの一般送配電事業者が公表する2025年の風力発電実績（30分値。年平均=1.0に正規化）、設備利用率（29.1%）とPPA発電単価（11.96円/kWh）は調達価格等算定委員会の資料を出典にしています。風力は送配電網で届く**オフサイト電源**として扱い、費用は2階建てです: ①発電した分にかかる支払（PPA発電単価＋発電側課金＋発電バランシング。全量払い/使用量払いを選べる）と、②送電ロスを経て実際に届いた分にかかる費用（託送の電力量料金・再エネ賦課金・小売グロスマージン）。**契約電力（基本料金）は下がらず**、売電できるのは敷地内の太陽光の余剰だけです。主役の出力は**量ベース達成率と時間一致率（24/7）**で、風力と蓄電池をどれだけ足すと24/7に近づくかを見られます。産業用・マイクログリッド（P-IRRを含む）とデータセンター、系統受電上限（DC）・蓄電池の最適容量探索とも併用できます（蓄電池の最適充放電LPは受電点の基準で最適化。受電上限は届く風力も含めた受電量に対する上限です）。太陽光（平年値）と風力（2025年実績）は別のデータのため、日単位の同時性は反映されません。詳細な費用構造・出典は `docs/wind_design_spec.md` §9 を参照。
