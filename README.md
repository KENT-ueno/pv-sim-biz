...
- **Wind power (offsite PPA)** — Add wind as a second generation source (solar only / wind only / both), for **Hokkaido and Tohoku only** (the demand site must be in the same area; cross-area wheeling is not modeled). Wind output = contract capacity × capacity factor × shape, where the shape is the 2025 half-hourly actual wind output of the area's transmission operator (normalized to mean 1.0) and the capacity factor (29.1%) and PPA generation price (¥11.96/kWh) come from the METI procurement-price committee. Wind is treated as an **offsite** source delivered over the grid, with a two-tier cost: (1) a generation-side payment (PPA price + grid-connection charge + generation balancing, billed on either the full generated amount or only the delivered amount depending on the payment basis) and (2) a delivered-energy charge (wheeling energy charge, renewable surcharge, retailer gross margin) on what actually reaches the demand site after transmission loss. The **contract demand does not fall**, and only onsite solar surplus can be sold. The headline outputs are the **volume-based vs hourly (24/7) matching rates**, which show how much wind and battery are needed to get close to 24/7. Works with the industrial/microgrid modes (incl. P-IRR), the data center mode, and the grid receiving cap. Solar (Typical Meteorological Year) and wind (2025 actuals) are different datasets, so day-level coincidence is not reflected.
...
- **風力発電（オフサイトPPA）** — 太陽光に加えて第2の発電源として使えます（太陽光のみ／風力発電（オフサイトPPA）のみ／併用）。**北海道・東北のみ**（需要地も同じエリアに限り、越境託送はモデル化していません）。発電量＝契約容量×設備利用率×形状で、形状は各エリアの一般送配電事業者が公表する2025年の風力発電実績（30分値。年平均=1.0に正規化）、設備利用率（29.1%）とPPA発電単価（11.96円/kWh）は調達価格等算定委員会の資料を出典にしています。風力発電（オフサイトPPA）は送配電網で届く電源として扱い、費用は2階建てです: ①風力発電（オフサイトPPA）の支払（PPA発電単価＋発電側課金＋発電バランシング。全量払い/使用量払いを選べる）と、②使用電力量（需要地に届く電力量＝発電量−送電ロス のうち、実際に使った量）にかかる託送の電力量料金・再エネ賦課金・小売グロスマージン。**契約電力（基本料金）は下がらず**、売電できるのは敷地内の太陽光の余剰だけです。結果の【年間の損得】では、導入しない場合（A）と導入した場合（B）の年間の支払を1行ずつ比べ、太陽光と風力発電（オフサイトPPA）それぞれの損得も示します。主役の出力は**量ベース達成率と時間一致率（24/7）**で、風力発電（オフサイトPPA）と蓄電池をどれだけ足すと24/7に近づくかを見られます。産業用・マイクログリッド（P-IRRを含む）とデータセンター、系統受電上限（DC）・蓄電池の最適容量探索とも併用できます（蓄電池の最適充放電LPは受電点の基準で最適化。受電上限は風力発電（オフサイトPPA）も含めた受電量に対する上限です）。太陽光（平年値）と風力発電（オフサイトPPA）（2025年実績）は別のデータのため、日単位の同時性は反映されません。詳細な費用構造・出典・用語は `docs/wind_design_spec.md` §9 を参照。
...
**Wind tools (Hokkaido / Tohoku only):** `list_wind_areas` / `estimate_wind_generation`。`validate_industrial_params` / `simulate_industrial_pv` / `validate_dc_params` / `simulate_dc` には任意引数 `wind`（`{"capacity_kw": 1000}` または `{"coverage_pct": 100}` ほか）と `pv_enabled` があり、省略すると従来と同じ結果です（風力発電（オフサイトPPA）を併用すると結果に `wind` 節・24/7の一致率・`electricity_cost.annual_balance`（年間の損得）が加わります）
...
- **一般送配電事業者「エリア需給実績」（北海道電力ネットワーク・東北電力ネットワーク）** — 風力発電実績・風力出力制御量（30分値）。風力発電（オフサイトPPA）の形状の出典
- **託送供給等約款（北海道電力ネットワーク・東北電力ネットワーク、2026年4月1日実施）** — 高圧・特別高圧の託送電力量料金・損失率・発電側課金（系統連系受電課金）の単価
- **JPEA「コーポレートPPA 日本の最新動向」報告書（2024年6月3日）** — フィジカルPPAの費用構造（発電側課金・送電ロス・発電インバランス・小売グロスマージン）。R4年度・高圧・太陽光の実績データ。小売グロスマージン（4.1円/kWh）・発電バランシング単価（1.1円/kWh）の暫定値の出典
