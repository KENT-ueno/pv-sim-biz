...
設計の正典は [`wind_design_spec.md`](wind_design_spec.md)、経緯は `decision_log.md` 第13段階（風力発電（オフサイトPPA）の追加）・W2f（費用構造の作り直し）。
ここでは実装の要点だけを記す。
...
風力発電（オフサイトPPA）は送配電網で届くので、敷地内の太陽光とは経済性が違う。**費用は2階建て**（`offsite_payment` / `offsite_receiving` / `offsite_cost_after`）:

```
需要地に届く電力量(t) = 発電量(t) × (1 − 損失率)                    ← 送電ロスは量で扱う（費用項目には足さない）
受電量  R(t)  = max(0, 需要 + 充電 − 放電 − 敷地内の太陽光)          ← 契約電力・受電上限の基準
使用電力量(t) = min(R(t) − import_(t), 需要地に届く電力量(t))       ← 30分ごとに計算

① 風力発電（オフサイトPPA）の支払（offsite_payment。既定=全量払いは発電量Gベース、使用量払いは使用電力量ベース）
   全量払い   = G×PPA発電単価 + 発電側課金(加算のとき) + G×発電バランシング単価
   使用量払い = 使用電力量 ×〔(PPA発電単価 + 発電側課金÷G) ÷ (1−損失率) + 発電バランシング単価〕
② 使用電力量にかかる託送・再エネ賦課金・小売グロスマージン = 使用電力量 ×（託送の電力量料金 + 再エネ賦課金 + 小売グロスマージン）

導入後の電気代 = 基本料金(R の最大) + 電力量料金(小売購入) + ②        （①は電気代の外。年間経済メリットから直接引く）
```

- **契約電力は風力発電（オフサイトPPA）では下がらない**（受電点の最大は、送配電網で届く風力発電（オフサイトPPA）も含む）。売電・出力抑制の対象は敷地内の太陽光の余剰だけ
...
- 蓄電池の**運転**は太陽光の発電量＋風力発電（オフサイトPPA）の**需要地に届く電力量**の合計で行い（風力発電（オフサイトPPA）を貯めて凪の時間に使う）、**料金だけ**受電点基準で計算し直す
- マイクログリッド（W2b）: 収益＝網内に供給した全量×網内単価＋束ねメリット、費用＝運営コスト＋風力発電（オフサイトPPA）の費用（①＋②）。
  PPA（MG）の単価は（投資の回収＋風力発電（オフサイトPPA）の費用）÷ 網内に供給した全量で逆算
- 設計・出典の一次資料・限界の詳細は `wind_design_spec.md` §9

### 14.3 Outputs / 出力

主役は **量ベース達成率**（年間の発電量 ÷ 年間の需要量）と **時間一致率**（1 − 系統購入/需要。24/7の実力）。両者の差が
「年間では足りていてもその時間には足りていない分」。蓄電池なしの参考（太陽光のみ／風力発電（オフサイトPPA）のみ／合計）と月別の表を出す。

### 14.4 Battery LP at the receiving point / 蓄電池LPを受電点の基準で解く（W2d）

風力発電（オフサイトPPA）（送配電網で届く電源）があるとき、蓄電池LP（最適充放電・最適容量探索）は**受電点の基準**で解く。
`optimize_battery` / `optimize_battery_capacity` の `offsite` 引数（`offsite_lp_spec` の戻り値。省略で従来のLPとビット同一）:

- `generation_30min` には敷地内の太陽光だけを渡す。変数は受電量 R（契約電力・受電上限の基準）と、風力発電（オフサイトPPA）の使用電力量 w（w ≤ R、w ≤ 需要地に届く電力量）
- 収支: R + PV + 放電 = 需要 + 充電 + 売電 + 抑制。売電・抑制はそのコマの太陽光の余剰以下（風力発電（オフサイトPPA）は売電できない）
- 目的: 基本料金×max(R) ＋ Σ((R−w)×小売単価 ＋ w×(託送＋再エネ賦課金＋小売グロスマージン)) − 売電。①（風力発電（オフサイトPPA）の支払）は、
  全量払いでは運転によらない定数なので目的関数に入れず、`offsite_payment` で事後に評価して最適容量探索の年間メリットから引く。
  使用量払いも実際の支払は事後に `offsite_payment` で評価する（運転の結果の使用電力量から計算）。ただし使用量払いだけ、目的関数に次の「判断用の上乗せ」を1つ入れる
- 使用量払いの上乗せ（`offsite_lp_surplus_penalty`。2026-09-26 のコードレビュー #4）: 使用量払いでも、届いた分の単価は全量払いと同じ（must-take。
  発電側単価まで見せると、LPが風力発電（オフサイトPPA）を全く受けなくなり、出なり供給の契約と合わない）。ただしそれだけだと、LPが風力発電（オフサイトPPA）を実際より安く見て蓄電池に貯める。
  オフサイトの電気は消費に先に配分されるので、風力発電（オフサイトPPA）に余剰電力量があるコマで、蓄電池なしでも受ける量
  base(t) ＝ min(需要地に届く電力量, max(0, 需要 − 太陽光)) を超えて受電すると、その分は風力発電（オフサイトPPA）になり発電側単価も払う。
  そこで、受電量のうち base を超える分に、使用量払いの発電側単価（`offsite_used_unit_yen`）を上乗せする。判断用で電気代ではない（`opt_annual_cost` からは除く）。
  全量払いには上乗せがない（従来のLPとビット同一）。既定の単価では、風力発電（オフサイトPPA）を蓄電池に貯める運転は起きない（詳細は `wind_design_spec.md` §9-11 の末尾）
- 受電上限は R の上限。風力発電（オフサイトPPA）は上限を守る助けにならないので、`diagnose_grid_cap` は太陽光だけで診断する
- 受電点では買電と売電が相殺される。売電単価が風力発電（オフサイトPPA）の使用電力量にかかる単価（託送・再エネ賦課金・小売グロスマージン）を上回ると、LPが「風力発電（オフサイトPPA）を受電して太陽光を売る」運転を選んでしまうので、
  **LP内の売電単価だけ**を抑え（`offsite_lp_sell_price`）、実際の収入は返ってきた運転を実際の単価で評価し直す
- ルールベース（W2e）: `simulate_battery(offsite_gen=…)`。蓄電池は太陽光の余剰だけを貯め、不足分を **風力発電（オフサイトPPA）→蓄電池→小売** の順に充てる
  （風力発電（オフサイトPPA）を貯めると受電量のピークが上がるため貯めない。風力発電（オフサイトPPA）を蓄電池で置き換えると、貯めずに売れた太陽光の余剰を捨てるだけなので放電は風力発電（オフサイトPPA）の後）。
  受電量は風力発電（オフサイトPPA）の使用電力量＋小売購入。風力発電（オフサイトPPA）を貯めて凪の時間に使う効果は、最適充放電（LP）でだけ出る
- 蓄電池なしは従来どおり太陽光の発電量＋風力発電（オフサイトPPA）の需要地に届く電力量の合計で計算し、料金だけ受電点の基準で計算し直す
