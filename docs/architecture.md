...
- ルールベース（W2e）: `simulate_battery(offsite_gen=…)`。蓄電池は太陽光の余剰だけを貯め、不足分を **風力（届いた分）→蓄電池→小売** の順に充てる
  （風力を貯めると受電量のピークが上がるため貯めない。風力を蓄電池で置き換えると、貯めずに売れた太陽光の余剰を捨てるだけなので放電は風力の後）。
  受電量は風力の配達分＋小売購入。風力を貯めて凪の時間に使う効果は、最適充放電（LP）でだけ出る
- 蓄電池なしは従来どおり太陽光＋風力の合計で計算し、料金だけ受電点の基準で計算し直す
