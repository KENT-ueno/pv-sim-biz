...
def get_station_options():
    ...
    df = pd.read_sql_query("""
        SELECT DISTINCT point_no AS station_no, point_name AS station_name
        FROM radiation_data
    """, conn)
    conn.close()
...
def process_and_plot(station, K, PAS, Ppeak, GS, alpha_pct, delta_T,
    ...
    try:
        ...
        effective_PAS = Ppeak / (K * G_STC) if Ppeak else PAS
        ...
        df_info = pd.read_sql_query("""
            SELECT point_lat AS latitude, point_lon AS longitude
            FROM radiation_data
            WHERE point_no = ?
        """, conn, params=(station_no,))
        conn.close()
        ...
        df_solar = df[df['element_no'] == '00001'].pivot_table(
            index=['month', 'day'], columns='hour', values='value').reset_index()
        ...
        times = pd.date_range("2020-01-01", periods=len(df_solar)*24, freq="H", tz="Asia/Tokyo")
        ...
        for h in range(1,25):
            df_hourly_nopv[h] = K * effective_PAS * df_solar_raw[h] * (1 + alpha * (df_temp[h] + delta_T)) / GS
            df_hourly_pvlib[h] = K * effective_PAS * df_solar[h]     * (1 + alpha * (df_temp[h] + delta_T)) / GS
        ...
        for h in range(1,25):
            raw_output = K * effective_PAS * df_solar[h] * (1 + alpha * (df_temp[h] + delta_T)) / GS
        ...
        # 24hグラフ
        df_day = df_hourly_corrected[(df_hourly_corrected['month'] == month_selected) &
